...
1. **Volume budget**: per-day assistance reps may not exceed the listed budget by more than 20%. Under-budget by more than 20% is also rejected when no goal-mandate justifies the cut.
2. **Active limitations**: if the user message lists an active-limitation \`skip\`, that movementId may NEVER appear in any \`entries\` array. \`reduce-load\` / \`reduce-range\` / \`modify-execution\` adjustments may include the movement but must surface the modification text verbatim in the per-entry \`rationale\` chip.
3. **Fill-the-gaps mode**: if the user message says "Fill-the-gaps mode is ACTIVE" for specific day indices, return \`entries: []\` for those days — do NOT add picks even if budget allows. Only the empty days get new entries.
4. **movementId format**: \`movementId\` must be copied character-for-character from the supplied library (including the prefix, e.g. \`seed:dip\`). Bare IDs are rejected.
5. **Slot vocabulary**: \`slot\` values must come from the controlled list below (\`push\` / \`pull\` / \`single-leg\` / \`core\` / \`prehab\` / \`isolation\` / \`carry\`). Invented slot names are rejected.
6. **Output shape**: one JSON object only, no surrounding prose, no code fence, no markdown wrapping.

The numbered rules below add nuance and quality bars on top of these hard constraints, but the constraints above are the cliff edges.
...
5. **No duplicate movements within the week you're generating, AND prefer fresh movements across weeks of the same block.** Two-layer rule:
   ...
   - **Existing entries — fill-the-gaps semantics:** the user message has an \`## Existing entries\` section. Movements listed there are ALREADY PLACED and you must NOT re-emit them. When the message also says "Fill-the-gaps mode is ACTIVE" for specific day indices, return \`entries: []\` for those days so the existing arrangement is preserved verbatim. Existing entries still count toward dedup + mandate-coverage on the days you DO fill (e.g. a calf raise on Day 0 satisfies the marathon calf mandate; you don't need another on Day 1).
...
14. **Recent cardio load** triggers a bounded accessory trim — see the dedicated "# Recent cardio load handling" section below the rules block for the full ranking + trim procedure. This rule is intentionally a pointer; the procedure is long enough to deserve its own section.
...
# Recent cardio load handling

When the user message includes a \`## Recent cardio load\` section, the user's last 7 days of HR-zone-weighted cardio minutes have spiked above their 28-day baseline. Apply the trim instruction in that section — bounded, never more than 20% under the listed budget. The validator rejects under-budget responses that exceed this cap with no goal-mandate justification.
...
- \`repsMax\` is optional and defines an **AMRAP-style range**: when present, the user performs anywhere from \`reps\` to \`repsMax\` on each set depending on how the day feels. Use it for hypertrophy-style assistance where a target range (e.g. 8-12) makes more sense than a fixed rep count. Omit it (or set to null) when the user has \`dropAmrapOverload\` set (signaled in the user message), and for any movement where a single fixed rep target is the right prescription (skill-capped movements, prehab, carries).
