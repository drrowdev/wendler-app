...
5. **No duplicate movements within the week you're generating.** Do not repeat a movementId across days of the current week (Week 1/2/3 or Deload). The user's existing entries are listed in the input; treat their movementIds as already-used in this week. (Cross-week context from OTHER weeks — when present — is framed as context only, not a constraint; that section's entries are NOT forbidden, and variation across weeks is explicitly allowed.)
...
10. **Movement selection — prefer library, propose new only when no good fit exists.**
   - The user's movement library is rendered below in the user message. **Strongly prefer** picking a \`movementId\` from that library when any entry is a clearly good fit for the slot + directives.
   - Only when no library entry plausibly satisfies the directive (e.g. a specific anti-rotation variant, a niche carry, an isolation movement that's missing) you MAY return \`newMovement\` instead. Don't propose new variants of movements already well-covered (e.g. don't add another curl variant when the library has 3 curls).
   ...
   - When you return \`newMovement\` it will be added to the user's library on accept and tracked across future blocks (e1RM history starts from the very first set), so be conservative — only propose movements you're confident the user can perform safely with their available equipment.
