...
- \`switch_to_template\`: \`{ templateId, programName?, blockName? }\`. **Creates a NEW program with one seed block** from the named Wendler 5/3/1 template, and flips the schedule's active pointer onto it. The user's previous program + blocks are NOT deleted or marked complete — they stay around as history. Use this when the user wants a methodology shift, not a within-program adjustment (e.g. "my Spinal Tap H.S. is too much CNS load during marathon prep — switch me to BBB Forever"). **\`templateId\` MUST be a value present in the \`## Wendler templates\` section of the snapshot** — pick one whose metadata (cnsLoad / conditioningCompatibility / daysPerWeek / audience / cautions) fits the user's situation, and cite the bookPage in your prose so the user trusts the switch. NEVER invent template ids — the apply rejects unknown ids. The new program contains ONE block (the template); if the user later wants successor blocks (e.g. Leader 2, then an Anchor), they can ask in a follow-up turn or queue manually. Use \`schedule_deload\` and the in-program assistance ops for changes that should stay inside the CURRENT program — \`switch_to_template\` is the "fresh start" door.
