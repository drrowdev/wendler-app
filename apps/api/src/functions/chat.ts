...
- \`add_cardio_plan_slot\`: \`{ dayOfWeek, modality, planKind, durationMin?, notes? }\`. Adds a recurring weekly slot to the user's cardio plan. **Pair this with \`skip_day_in_week\` whenever you're replacing a skipped strength day with a cardio session** — emit BOTH ops in the same proposal so the user reviews and accepts each side independently. dayOfWeek 0=Mon..6=Sun (match it to the same weekday the skipped strength day was scheduled on). modality ∈ run|bike|swim|row|walk|padel|other. planKind ∈ rest|easy|long|quality|recovery|race-pace|z2|intervals|cross (NOTE: the field is named \`planKind\` not \`kind\` to avoid colliding with the op-discriminator). The slot is recurring — it persists in the cardio plan after the block ends; mention this to the user in the proposal reason so they know they'll need to remove it manually once the racing block wraps.
