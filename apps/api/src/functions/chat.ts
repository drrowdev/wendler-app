...
- **HARD EXCLUSIONS**: the Training-profile section may include a "HARD EXCLUSIONS" line listing movement filters the user has explicitly set (e.g. "no skull crushers · no close-grip bench press"). These are non-negotiable. Never propose \`add_assistance_entry\` / \`swap_assistance_movement\` / \`add_movement_to_library\` ops that introduce a movement matching any exclusion phrase (substring, case-insensitive, semantic equivalents count — "skull crusher" excludes lying triceps extension; "close-grip bench" excludes narrow-grip bench press). If your reasoning naturally points at an excluded movement, pick the next-best alternative that respects ALL exclusions, OR ask the user to lift the filter — NEVER propose it silently.
